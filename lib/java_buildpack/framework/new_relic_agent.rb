...
module JavaBuildpack
  module Framework
    ...
    class NewRelicAgent < JavaBuildpack::Component::VersionedDependencyComponent
      ...
      def release
		@logger            = Logging::LoggerFactory.instance.get_logger NewRelicAgent
		@logger.debug { "AT : Within release  #{(Time.now.to_f.round(3)*1000).to_i}" }
      ...
      def supports?
		@logger            = Logging::LoggerFactory.instance.get_logger NewRelicAgent
		@logger.debug { "AT : Within supports  #{(Time.now.to_f.round(3)*1000).to_i}" }
      ...
      def apply_configuration(credentials, configuration)
		@logger            = Logging::LoggerFactory.instance.get_logger NewRelicAgent
		@logger.debug { "AT : Within apply_configuration  #{(Time.now.to_f.round(3)*1000).to_i}" }
      ...
      def apply_user_configuration(credentials, configuration)
		@logger            = Logging::LoggerFactory.instance.get_logger NewRelicAgent
		@logger.debug { "AT : Within apply_user_configuration  #{(Time.now.to_f.round(3)*1000).to_i}" }
      ...
      def write_java_opts(java_opts, configuration)
		@logger            = Logging::LoggerFactory.instance.get_logger NewRelicAgent
		@logger.debug { "AT : Within write_java_opts #{(Time.now.to_f.round(3)*1000).to_i}" }
